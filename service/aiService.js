...
exports.sendFinalToOpenAi = async(finalImageBuffer, finalDrawingBuffer, testId, type) => {
    try {
        ...
        const prompt = `${objectDescription} 그림 분석 - 역순 분석 방법:

**다음 단계로만 분석:**

1. **시간 추출 및 정렬:**
   - 좌표 데이터에서 모든 strokeStartTime 값을 추출
   - 오름차순으로 정렬해서 시간 리스트 만들기
   - 예: [2917, 3200, 3500, 5800, 6100, 8200, ...]

2. **시간순 객체 매칭:**
   - 정렬된 시간 순서대로 각 stroke가 어떤 객체를 그리고 있는지 판단
   - 예: 
     * 2917ms의 stroke → 이미지 보니 지붕 그리기
     * 3200ms의 stroke → 이미지 보니 지붕 그리기 (이미 기록됨)
     * 5800ms의 stroke → 이미지 보니 벽 그리기 (새로운 객체!)

3. **첫 등장 시간만 기록:**
   - 각 객체가 처음 나타나는 시간만 기록
   - strokeStartTime ÷ 1000 = 초, 소수점 버림해서 MM:SS 변환

4. **최종 결과:**
   - 지붕 첫 시작: 2917ms → 00:02
   - 벽 첫 시작: 5800ms → 00:05
