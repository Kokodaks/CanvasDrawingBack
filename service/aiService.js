...
exports.sendFinalToOpenAi = async(finalImageBuffer, finalDrawingBuffer, type, testId) =>{
    ...
    3. **객체와 획 매핑 규칙**:
        - 각 획(stroke)은 반드시 하나의 객체에만 할당되어야 합니다
        - 한 획이 여러 객체의 시작점으로 중복 사용되지 않도록 주의하세요
        - 한 객체의 시작 획으로 이미 사용된 획은 다른 객체의 시작 획으로 사용하지 마세요
        - 각 객체마다 가장 먼저 그려진 획을 찾아 해당 객체의 시작 시간으로 지정하세요
        - 객체들을 시간 순서대로 정렬할 때, 이미 사용된 획은 제외하고 남은 획들 중에서 다음 객체의 시작 획을 찾으세요
    ...
    5. **응답 포맷 가이드**:
        ...
        - 중복된 타임스탬프가 있으면 안 됩니다. 각 객체는 고유한 시작 시간을 가져야 합니다.
    ...
    9. 각 획은 하나의 객체에만 할당하세요. 이미 한 객체의 시작으로 사용된 획은 다른 객체의 시작으로 재사용하지 마세요.
